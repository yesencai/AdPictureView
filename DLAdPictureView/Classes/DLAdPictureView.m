...
@interface DLAdPictureView ()<UIScrollViewDelegate>
{
...
@implementation DLAdPictureView

+ (instancetype)picViewWithLoadImageBlock: (LoadImageBlock)loadBlock {
    
    DLAdPictureView *adView = [[DLAdPictureView alloc] init];
...
-(void)setPicModels:(NSArray<id <DLAdPictureProtocol>> *)picModels
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<DLAdPictureProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<DLAdPictureProtocol> adM = self.picModels[tag];
